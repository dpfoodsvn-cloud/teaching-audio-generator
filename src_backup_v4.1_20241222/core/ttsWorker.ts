...
export class TTSWorker {
    ...
    static async generateAudio(
        ...
        // Inject style prompt - TTS model expects clear text to read
        // Prefix with instruction to ensure model treats this as TTS, not text generation
        const ttsPrefix = "Read the following text aloud naturally: ";
        const fullPrompt = ttsPrefix + text;
            ...
            if (isRetryable && retryCount < maxRetries) {
                ...
                if (retryMatch) {
                    ...
                    if (availableKeys.length > 0) {
                        console.log(`[Smart Rotation] Switching to next available key immediately (${availableKeys.length} available)`);
